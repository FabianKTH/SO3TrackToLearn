...
assert len(sys.argv) == 1 or len(sys.argv) == 3, (
    'usage: <name.py> <bool: so3_rotate_state_neighb> <bool: so3_rotate_equiv_graph>')
if len(sys.argv) == 3:
    if sys.argv[1].lower() == 'true':
        rot_state = 'rs'
        exp1_argv += ['--so3_rotate_state_neighb']
...
for rng_seed in seeds:
    ...
    argv += [f'--neighborhood_mode=star',
             ...
             f'--run_tractometer',
             f'--use_comet',
             f'--n_actor={2**12}',              # directly affects batchsize
             f'--lr={0.0005}',
             ]
