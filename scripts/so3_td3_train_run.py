...
dataset_file=join(WORK_DATASET_FOLDER, 'raw_fabi_2024', SUBJECT_ID, SUBJECT_ID + '-mrtrix.hdf5')
test_dataset_file=join(WORK_DATASET_FOLDER, 'raw_fabi_2024', TEST_SUBJECT_ID, TEST_SUBJECT_ID + '-mrtrix.hdf5')
test_reference_file=join(WORK_DATASET_FOLDER, 'raw_fabi_2024', TEST_SUBJECT_ID, 'masks', TEST_SUBJECT_ID + '_wm.nii.gz')

max_ep=500 # 1000 # Chosen empirically
log_interval=25 # Log at n steps
lr=0.0005 # (0.0005) Learning rate
gamma=0.5 # 0.5 ? Gamma for reward discounting
alpha=0.2

valid_noise=0.0 # Noise to add to make a prob output. 0 for deterministic

n_seeds_per_voxel=20 # (2) Seed per voxel
max_angle=30 # (30) Maximum angle for streamline curvature

EXPERIMENT='SO3TD3FiberCupTrain'
...
for rng_seed in seeds:
  ...
  argv = [f'{TTL_ROOT}/TrackToLearn/trainers/so3_td3_train.py',
   DEST_FOLDER,
   EXPERIMENT,
   ID,
   dataset_file,
   ...
   f'--use_comet',
   f'--n_actor=512',
   f'--add_neighborhood=2.5',
   f'--angle_penalty_factor=2',
   f'--n_dirs=2'
   ]
    #    f'--target_bonus_factor=2']
    # f'--straightness_weighting=2',
    # f'--n_actor={128}',
    # --render
    # f'--use_comet',
