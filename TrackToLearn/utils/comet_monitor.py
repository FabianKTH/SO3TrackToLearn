...
class CometMonitor():
    ...
    def update(
        ...
        if vc_monitor is not None and len(vc_monitor) > 0:
            ...
            if eqiv_monitor is not None and len(eqiv_monitor) > 0:
                eq_x, eq_y = zip(*eqiv_monitor.epochs)
                # metric_dict[self.prefix + "EQUIVARIANCE"] = eq_y[-1]
                self.e.log_metrics(
                    {self.prefix + "EQUIVARIANCE": eq_y[-1]},
                    step=i_episode
                    )
