...
class Tracker(object):
        ...
        def tracking_generator():
            ...
            for i, start in enumerate(
                ...
                self.alg.validation_episode(
                    state, self.env, skip_equiv_test=skip_equiv_test)
                batch_tractogram = self.env.get_streamlines()

                # next line, additional check for back_env to be able to run
                # without backwards tracking at all.
                if not self.interface_seeding and self.back_env:
                    state = self.back_env.reset(batch_tractogram.streamlines)

                    # Track backwards
                    self.alg.validation_episode(
                        state, self.back_env, skip_equiv_test=skip_equiv_test)
    ...
    def track_and_validate(
        ...
        # Equivariance metrics observed during tracking
        equiv_hist = []
        ...
        def _generate_streamlines_and_rewards():
            ...
            for i, start in enumerate(
                ...
                # Track forward
                reward, equiv = self.alg.validation_episode(state, self.env)
                batch_tractogram = self.env.get_streamlines()
                ...
                if not self.interface_seeding:
                    ...
                    # Track backwards
                    reward, equiv = self.alg.validation_episode(
                        state, self.back_env)
                    batch_tractogram = self.back_env.get_streamlines()

                yield batch_tractogram, reward, equiv

        for t, r, e in _generate_streamlines_and_rewards():
        ...
        return tractogram, cummulative_reward, np.mean(equiv_hist)
