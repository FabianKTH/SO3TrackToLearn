...
class TrackToLearnTraining(TrackToLearnExperiment):
    ...
    def __init__(
        ...
        # fabi
        self.init_dirs_from_peaks = train_dto['init_dirs_from_peaks']
        self.skip_validation = train_dto.get('skip_validation', False)
        ...
        self.neighborhood_mode = train_dto['neighborhood_mode']
        ...
        # SO3 parameters
        self.so3_critic_lr = train_dto.get('so3_critic_lr', None)
        self.so3_critic_hidden_dims = train_dto.get('so3_critic_hidden_dims', None)
        self.so3_actor_lr = train_dto.get('so3_actor_lr', None)
        self.so3_spheredir = train_dto.get('so3_spheredir', None)
        self.so3_actor_input_irrep = train_dto.get('so3_actor_input_irrep', None)
        self.so3_neighb_from_peaks = train_dto.get("so3_neighb_from_peaks", False)
        self.so3_rotate_equiv_graph = train_dto.get("so3_rotate_equiv_graph", False)
        self.so3_actor_hidden_lmax = train_dto.get('so3_actor_hidden_lmax', None)
        self.so3_actor_hidden_depth = train_dto.get('so3_actor_hidden_depth', None)
        ...
        self.hyperparameters = {
            ...
            # 'complexity_weighting': self.complexity_weighting,
            # so3 parameters
           'so3_critic_lr': self.so3_critic_lr,
           'so3_critic_hidden_dims': self.so3_critic_hidden_dims,
           'so3_actor_lr': self.so3_actor_lr,
           'so3_spheredir': self.so3_spheredir,
           'so3_actor_input_irrep': self.so3_actor_input_irrep,
           'so3_neighb_from_peaks': self.so3_neighb_from_peaks,
           'so3_rotate_equiv_graph': self.so3_rotate_equiv_graph,
           'so3_actor_hidden_lmax:': self.so3_actor_hidden_lmax,
           'so3_actor_hidden_depth:': self.so3_actor_hidden_depth,
        }
    ...
    def save_model(self, alg, model_name='last_model_state'):

        directory = pjoin(self.experiment_path, "model")
        if not os.path.exists(directory):
            os.makedirs(directory)
        alg.policy.save(directory, model_name)
    ...
    def rl_train(
        ...
        if not skip_validation:
            ...
            valid_tractogram, valid_reward, valid_equiv = valid_tracker.track_and_validate()
            scores = self.score_tractogram(valid_tractogram)
            self.save_model(alg)

            # Display the results of the untrained network
            self.log(valid_tractogram, env, valid_reward, i_episode,
                     scores=scores, valid_equiv=valid_equiv)
        ...
        while i_episode < self.max_ep:
            ...
            if i_episode % self.log_interval == 0:
                ...
                if not skip_validation:
                    # Validation run
                    valid_tractogram, valid_reward, valid_equiv = \
                ...
                self.save_model(alg, model_name=f'training_ep-{i_episode}')
