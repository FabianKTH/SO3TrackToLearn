...
from TrackToLearn.environments.seedtracking_env import SeedTrackingEnvironment
...
class TrackToLearnExperiment(Experiment):
    ...
    def setup_monitors(self):
        ...
        self.equiv_monitor = LossHistory(
            "Validation Equivariance", "valid_equiv", self.experiment_path)
    ...
    def setup_comet(self, prefix=''):
        ...
        self.comet_monitor = CometMonitor(
            ...
            prefix, self.render, log_equiv=True)

        self.comet_monitor.log_parameters(self.hyperparameters)

    def _get_env_dict_and_dto(
        self, interface_tracker, no_retrack, noisy, seeds_provided=False
    ) -> Tuple[dict, dict]:
        ...
        env_dto = {
            ...
            'init_dirs_from_peaks': self.init_dirs_from_peaks,
        ...
        if seeds_provided:
            class_dict = {
                    'tracker': SeedTrackingEnvironment,
    ...
    def log(
        ...
        print('Episode {} \t avg length: {} \t total reward: {} \t avg equiv: {}'.format(
            i_episode,
            avg_length,
            avg_valid_reward,
            valid_equiv
        ...
        if scores is not None and self.run_tractometer:
        ...
        # update equivariance
        self.equiv_monitor.update(valid_equiv)
        self.equiv_monitor.end_epoch(i_episode)
        ...
        if self.use_comet and self.comet_experiment is not None:
            ...
            self.comet_monitor.update(
                ...
                self.ol_monitor,
                i_episode=i_episode,
                eqiv_monitor=self.equiv_monitor
                )
